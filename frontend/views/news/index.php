    ...
    <div class="container">
       <div class="news-nav clearfix">
           <div class="news-type pull-right">
               <a id='grid-list' href="<?php if($language=='en'){echo '/en';}?>/news/grid/<?=$id?>" title="<?php echo \Yii::t('app', '瀑布流模式');?>"><i class="icon icon-grid"></i></a>
               <a id="normal-list" class="active" href="<?php if($language=='en'){echo '/en';}?>/news/list/<?=$id?>" title="<?php echo \Yii::t('app', '列表模式');?>"><i class="icon icon-list"></i></a>
           </div>
           <div  class="news-tags">
       ...
       <div class="news-list normal" id="news_list"  <?php if(!isset($news_res)||!is_array($news_res)||count($news_res)==0){?>style="min-height: 400px"<?php }?>>
