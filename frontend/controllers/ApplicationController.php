...
class ApplicationController extends BaseController
{
    ...
    {
            $postdata['name'] = Yii::$app->request->post('name','小王');
            $postdata['cellphone'] = Yii::$app->request->post('cellphone','15764246577');
            $postdata['certificate_type'] = Yii::$app->request->post('certificate_type',0);
            $postdata['certificate_number'] = Yii::$app->request->post('certificate_number','370683199404222927');
            $postdata['email'] = Yii::$app->request->post('email','809061002@qq.com');
            $postdata['company'] = Yii::$app->request->post('company','能见');
            $postdata['position'] = Yii::$app->request->post('position','php');
            if($postdata['name']==''){
                    $this->renderJson(999,[],'姓名不能为空');
            }
            if($postdata['cellphone']==''){
        ...
        //print_r($postdata);exit;
        $application_model = new Application();
        $add_res = $application_model->testAdd($postdata);
    }
}
